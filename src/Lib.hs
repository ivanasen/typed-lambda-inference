...
applySubstitution sub (Function src dst)
    = Function
        (applySubstitution sub src)
        (applySubstitution sub dst)
...
inferTypeUtil ctx (EFunc arg body) = do
    ...
    Just (Function (applySubstitution subst newVariable) bodyType, subst)
...
typeToString (Primitive name) = name
typeToString (Variable name) = show name
typeToString (Function src dst)
    = "(" ++ typeToString src ++ " -> " ++ typeToString dst ++ ")"
